# Contributing to WomenInSoftwareDev
...
1. Search [WomenInSoftwareDev](https://github.com/WomenInSoftwareDev/womeninsoftware.dev/pulls) for an open or closed PR that relates to your submission.
...
3. [Fork](https://docs.github.com/en/github/getting-started-with-github/fork-a-repo) the `womeninsoftware.dev` repo.
...
10. In GitHub, send a pull request to `WomenInSoftwareDev/womeninsoftware.dev:main`.
...
If the issue you are working on is an approved request for adding a new profile to be featured in **WomenInSoftwareDev**, follow these steps:

1. Make sure that an issue exists for the profile and it has been approved by looking for the `approved` label on the issue.

2. Add the profile at the end of the JSON array of profiles in the `profiles.json` file located in `public/data`.

3. Ensure the profile details are correctly formatted and include all necessary information as per the existing entries.

4. If some data is missing, collect the data before submitting the PR. PRs for profiles with missing data will not be accepted.
...
6. Add a featured image 
   ...
   - press `F11` so that the browser contents take full width
   - take a full-width `16x9` aspect ratio screenshot using any screenshot tool
   - add the image to the `public/images/profiles` folder

7. Include any relevant tests to verify the new profile entry if applicable.

Following these steps will ensure that new profiles are added correctly and maintain the integrity of the data used by the application.
